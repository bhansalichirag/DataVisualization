import { scalePow, scaleLinear, scaleBand } from "d3-scale";
...
const User = {
    ...
    load: function() {
        ...
        this.yelpAgeScale = scalePow()
            .range([5, this.height - this.padding])
            .domain([3.85, 10.81]);

        this.reviewsScale = scalePow()
            .exponent(1/3)
            .range([5, this.height - this.padding])
            .domain([6.07, 633.42]);

        this.coolnessScale = scalePow()
            .exponent(1/3)
            .range([5, this.height - this.padding])
            .domain([0.75, 5518]);

        this.fansScale = scalePow()
            .exponent(1/3)
            .range([0, this.height - this.padding])
            .domain([0, 89.75]);

        this.starsScale = scalePow()
            .range([1, this.height - this.padding])
            .domain([1.57, 4.72]);

        this.usefulScale = scalePow()
            .exponent(1/3)
            .range([5, this.height - this.padding])
            .domain([3, 6487.5]);
        ...
        container = select(".userstat-container");
        container.append("h3")
        ...
        this.svg.append("g")
            ...
            .call(axisLeft(this.yScale).tickSize(2));
        
        this.svg.append("g")
            .attr("transform", `translate(5, ${this.height - this.padding})`)
            .call(axisBottom(this.xScale).tickFormat((d) => d).tickSize(2));
    ...
    updateInfo: function(data) {
        ...
        rects.transition()
            ...
            .attr("fill", (d) => interpolateGreens(this.getScaledValue(d[0], d[1]) / (this.height - this.padding)));
