...
import { schemeSet3, schemeSet1, schemeCategory10, schemeTableau10 } from "d3-scale-chromatic";
import Map from "./map";
...
const BubbleChart = {
    ...
    load: function() {
        ...
        this.color = scaleOrdinal(this.data.map(d => d.name), [...schemeSet3, ...schemeSet1, ...schemeCategory10, ...schemeTableau10]);
        this.format = format(",d");
                    
        this.root = this.pack(this.data);

        this.svg = select(".bubble-chart-container")
            .append("svg")
            .attr("font-size", 10)
